...
export const AddIncome = async (payload) => {
    try {
        ...
        const { data } = await api.post(`/income/add-incomes/${userId}`, payload);
...
export const deleteIncome = async (userId, expenseId) => {
    try {
        ...
        const { data } = await api.delete(`/income/delete-incomes/${userId}/${expenseId}`);
...
export const UpdateIncome = async (userId, expenseId,payload) => {
    try {
        console.log(userId, expenseId)
        const { data } = await api.patch(`/income/update-incomes/${userId}/${expenseId}`,payload);
