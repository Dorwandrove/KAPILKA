...
const addExpense = async (req, res) => {
    try {
        ...
        if (req.user.id !== req.params.userId)//בדיקה שהבקשה התבצעה באמת על ידי מי שמחובר כקליינט
        ...
        if (currency !== "ILS") {
            ...
            originalAmount = data.conversion_rates.ILS * amount;ז //הסכום המקורי אחרי המרה לשקלים
        }
...
const updateExpense = async (req, res) => {
    try {
        ...
        const expense = await Expense.findOneAndUpdate(
            { _id: expenseId },
            { title, description, amount, tag, currency,originalAmount },//הפרמטרים שנתונים לעדכון
            { new: true }
        );
